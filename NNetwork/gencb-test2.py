...
def create_classification_model(temporal_input_shape, spatial_input_dim, spatial_embedding_dim=13, learning_rate=0.001):
...
def predict_one_week_ahead(crime_time_series, timesteps, scaler_temporal_final, final_model, le_census_block):
    ...
    for block in blocks:
        ...
        # Determine the prediction date (start date of the next week)
        last_date = recent_data['date'].iloc[-1]
        # Prediction date is the next week's end date (since we're grouping by weeks ending on Sundays)
        prediction_date = pd.to_datetime(last_date) + pd.Timedelta(days=7)
    ...
    original_census_blocks = le_census_block.inverse_transform(X_spatial_pred.flatten().astype(int))

    # Create DataFrame with predictions
    prediction_results = pd.DataFrame({
        'census_block_numeric': X_spatial_pred.flatten(),
