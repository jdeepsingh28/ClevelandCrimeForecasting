...
def shape_data(data, timesteps):
    ...
    for block in blocks:
        ...
        for i in range(len(X_temporal) - timesteps + 1):
            ...
            X_spatial_seq.append(X_spatial[i + timesteps - 1])
...
def modeling_with_scaling_classification(
    ...
    X_train_spatial_scaled = X_train_spatial
    X_val_spatial_scaled = X_val_spatial
...
def evaluate_model(model, scaler_temporal, X_test_temporal, X_test_spatial, y_test):
    """
    Evaluates the model on the test set, prints out metrics, and plots ROC curves.
    """
    ...
    X_test_spatial_scaled = X_test_spatial
...
def main():
    # Step 1: Load Data
    file_path = r"\Users\singh\Downloads\crime_with_weather.csv"
    ...
    # Ensures crime_time_series is always sorted properly
    ...
    timesteps = 1  # Determines how many weeks the model will look back
